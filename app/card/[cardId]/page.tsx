...
export default function LoyaltyCardPage({ params }: PageProps) {
  ...
  const handleAddToGoogleWallet = async () => {
    ...
    try {
      const res = await fetch(`/api/loyalty/wallet/google?clientId=${client.qr_code_data}`);
      const data = await res.json();

      if (data.saveUrl) {
        window.open(data.saveUrl, '_blank');
      } else if (data.configured === false) {
        alert('Google Wallet n\'est pas encore configuré pour ce commerce.');
      } else {
  ...
  const handleAddToAppleWallet = async () => {
    ...
    try {
      const res = await fetch(`/api/loyalty/wallet/apple?clientId=${client.qr_code_data}`);
      const contentType = res.headers.get('content-type') || '';

      if (contentType.includes('application/vnd.apple.pkpass')) {
        // .pkpass file returned - download it
        const blob = await res.blob();
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `${shopName.replace(/\s+/g, '_')}_loyalty.pkpass`;
        ...
        if (data.configured === false) {
          alert('Apple Wallet n\'est pas encore configuré pour ce commerce.');
        } else {
          alert('La fonctionnalité Apple Wallet sera bientôt disponible.');
        }
      }
    } catch {
      alert('Erreur de connexion. Veuillez réessayer.');
                  ...
                  <div className="flex flex-col md:flex-row items-center gap-8">
                    ...
                    <div className="flex-1 space-y-3 w-full md:max-w-xs">
                      ...
                      <div className="pt-2 border-t border-slate-100 space-y-2">
                        {/* Apple Wallet - show first on iOS, second otherwise */}
                        {(isIOS || !isAndroid) && (
                          <button
                            onClick={handleAddToAppleWallet}
                            disabled={walletLoading === 'apple'}
                            className="w-full flex items-center justify-center gap-2 bg-black hover:bg-gray-800 text-white rounded-xl py-3 px-4 transition-colors disabled:opacity-50"
                          >
                            {walletLoading === 'apple' ? (
                        ...
                        {/* Google Wallet - show first on Android, second otherwise */}
                        {(isAndroid || !isIOS) && (
                          <button
                            onClick={handleAddToGoogleWallet}
                            disabled={walletLoading === 'google'}
                            className="w-full flex items-center justify-center gap-2 bg-white hover:bg-gray-50 text-slate-800 border border-slate-200 rounded-xl py-3 px-4 transition-colors disabled:opacity-50 shadow-sm"
                          >
